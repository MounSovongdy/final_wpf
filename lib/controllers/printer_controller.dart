...
Future<String> generateHtmlContent() async {
  // Load the Khmer font
  final fontBytes = await rootBundle.load('assets/fonts/NotoSansKhmer-Regular.ttf');
  final fontBase64 = base64.encode(fontBytes.buffer.asUint8List());

  const name = 'សួស្តី, Sovongdy';

  // HTML content with embedded Khmer font
  return '''
...
void downloadPdf() async {
  ...
  // Inject HTML content into the DOM
  final div = html.DivElement()..setInnerHtml(htmlContent, treeSanitizer: html.NodeTreeSanitizer.trusted);
  html.document.body?.append(div); // Append to the body for html2pdf to access

  // Call the JavaScript function to download the PDF
  try {
    js.context.callMethod('downloadPDF');
  } catch (e) {
    print('Error calling downloadPDF: $e');
  }

  // Remove the div after generating the PDF to clean up
  div.remove();
}
...
void printPdf() async {
  ...
  html.document.body?.append(div); // Append to the body for printing

  // Call the JavaScript function to print the content
  try {
    js.context.callMethod('printContent');
  } catch (e) {
    print('Error calling printContent: $e');
  }

  // Wait for the print dialog to open and then remove the div
  await Future.delayed(Duration(seconds: 1)); // Allow some time for printing
  div.remove(); // Clean up
}